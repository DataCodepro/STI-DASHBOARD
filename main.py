...
with header:
    ...
    st.subheader('STI INDICATORS DASHBOARD')
    if st.checkbox('SHOW INFORMATION ABOUT DATASET BEEN USED'):
...
with dataset:
    ...
    df.sort_values('sinfo1',ignore_index=True,inplace=True)
    ...
    'SECTORS AND THEIR INFORMATION SOURCE -PROFESSIONAL, INDUSTRY ASSOCIATIONS','EFFECT OF INNOVATION (PRODUCT) - INCREASED RANGE OF GOODS/SERVICE OF EACH SECTORS','Bubble'],key=1)
    if st.checkbox('Show Plot'):
        ...
        paper_bgcolor="#4233FF",)
            ...
            fig = px.sunburst(df2, path=['year','service', 'sector'], values='turnover07',width=800, height=500,color='turnover07', 
            color_continuous_scale='rainbow',color_continuous_midpoint=np.mean(df2['turnover07']))
            fig.update_traces(textinfo='label+percent entry') 
            fig.update_layout(margin= dict(l=20, r=20, t=20, b=20),
                                paper_bgcolor="#4233FF ",)
            ...
            fig = px.scatter(df, y ='totalstaff06',x ='turnover06', animation_frame='year',
            animation_group='sector', color = 'service',hover_name = 'sector',
            log_x=True,size_max = 10000,range_y=[0,100])
            st.plotly_chart(fig)
